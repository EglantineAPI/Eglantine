...
const (
	...
	caveStride = 4
	// caveFieldTop is the highest y the lattice covers. Nothing generated by
	// this generator reaches it.
	caveFieldTop = 256
)
...
// newCaveField samples the cave noise around a chunk. The lattice extends one
// point past the chunk on each side so every block inside it can interpolate.
func (o *Overworld) newCaveField(pos chunkPos) *caveField {
	f := &caveField{
		x0: pos.x * 16, z0: pos.z * 16, y0: caveFloor,
		nx: 16/caveStride + 1, nz: 16/caveStride + 1,
	}
	f.ny = (caveFieldTop-f.y0)/caveStride + 2
